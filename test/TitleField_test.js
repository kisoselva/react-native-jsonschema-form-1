...
describe("TitleField", () => {
  ...
  const getTextComponent = (node, getRequiredComponent = false) => 
    node.querySelectorAll("text")[getRequiredComponent ? 1 : 0];
  
  const getTextContent = (node, getRequiredComponent = false) =>
    getTextComponent(node, getRequiredComponent).props.children[0];
  ...
  it("should return a legend", () => {
    ...
    expect(getTextContent(node)).to.equal("Field title");
  });
  ...
  it("should have the expected id", () => {
    ...
    expect(getTextComponent(node).props.id).to.equal("sample_id");
  });
  ...
  it("should include only title, when field is not required", () => {
    ...
    expect(getTextContent(node)).to.equal(props.title);
  });
  ...
  it("should add an asterisk to the title, when field is required", () => {
    ...
    expect(getTextContent(node, true)).to.equal("*");
  });
});
